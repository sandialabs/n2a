...
public abstract class UnitFormat extends Format {
    ...
    public static UnitFormat getInstance(Locale inLocale) {
        return UnitFormat.ASCII; //DEFAULT; // TBD: Implement Locale Format. 
    }
    ...
    static {
        ...
        for (int i = 0; i < PREFIXES.length; i++) {
            ...
            DEFAULT.label(SI.KILOGRAM.transform(CONVERTERS[i].concatenate(Em3)),
                        PREFIXES[i] + "g");
            if (PREFIXES[i] == "µ") {
                 ASCII.label(SI.KILOGRAM.transform(CONVERTERS[i].concatenate(Em3)), "ug");
            }   
        }
    ...
    static {
        ...
        ASCII.label(SI.MICRO(NonSI.LITRE), "uL");
